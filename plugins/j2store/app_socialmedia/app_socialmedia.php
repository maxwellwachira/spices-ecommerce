...
class plgJ2StoreApp_socialmedia extends J2StoreAppPlugin
{
    ...
    function getImages($product){
        ...
        $share_product_image_type = $this->params->get('share_product_image_type',0);
        if($share_product_image_type){
        ...
        if (!empty($product_image)) {
            return JURI::root().$product_image;
        } else{
    ...
    function getProductName($product){
        if (!isset($product->j2store_product_id)) {
            ...
            return addslashes($product_name);
        }
    }
    ...
    public function onContentAfterTitle($context,$items,$params){
        $product = $items;
        if(!isset($items->pricing->price)){
            $product_table = F0FTable::getAnInstance('Product','J2StoreTable')->getClone();
            $product = $product_table->get_product_by_source('com_content',$items->id);

            $product_helper = J2Store::product();

            //get product
            $product = $product_helper->setId($product->j2store_product_id)->getProduct();
            F0FModel::getTmpInstance('Products', 'J2StoreModel')->runMyBehaviorFlag(true)->getProduct($product);
        }
        return $this->displaySocialPlugins('afterDisplayTitle',$product);
    }
    ...
    public function onContentBeforeDisplay($context,$items,$params){
        ...
        return $this->displaySocialPlugins('beforeDisplayContent',$product);
    }
    ...
    public function onContentAfterDisplay($context,$items,$params){
        ...
        return $this->displaySocialPlugins('afterDisplayContent',$product);
    }
    ...
    public function onJ2StoreViewProduct($product, $view){
        ...
        if($this->params->get('show_facebook_icon',1)){
            $document->setMetaData('fb:app_id',$this->params->get('facebook_app_id',0),'property');
            $document->setMetaData('og:type', 'product','property');
            //  $document->setMetaData('og:locale', 'product');
            $document->setMetaData('og:title', addslashes($product->product_name),'property');

            if($image_url ){
                $document->setMetaData('og:image', $image_url,'property');
            }
            //$document->setMetaData('product:description', strip_tags($document->getDescription()));

            //price with tax
            $document->setMetaData('product:original_price:amount', $product->pricing->base_price,'property');
            $document->setMetaData('product:original_price:currency', J2Store::currency()->getCode(),'property' );

            //without tax
            $document->setMetaData('product:pretax_price:amount', $product->pricing->base_price,'property');
            $document->setMetaData('product:pretax_price:currency',J2Store::currency()->getCode(),'property');

            $document->setMetaData('product:weight:value', $product->variant->weight,'property');
            $document->setMetaData('product:weight:units', $product->variant->weight_unit,'property');

            //sale prices
            $document->setMetaData('product:sale_price:amount', $product->pricing->price,'property');
            $document->setMetaData('product:sale_price:currency', J2Store::currency()->getCode(),'property');
       ...
       */
            ...
            $document->setMetaData('product:availability',$availability,'property');
            $document->setMetaData('product:brand',$product->manufacturer ,'property');

            $document->setMetaData('product:category',$product->source->category_title,'property');
            /* $document->setMetaData('product:color', 'product'); */
            $document->setMetaData('product:isbn', $product->variant->upc,'property');
            $document->setMetaData('product:upc', $product->variant->upc,'property');
        }
        ...
        if($this->params->get('twitter_meta_tags',1)){
            ...
            if($this->params->get('twitter_add_card_meta',1)){
                ...
                $document->setMetaData('twitter:title', addslashes($product->product_name) . $formated_price);
