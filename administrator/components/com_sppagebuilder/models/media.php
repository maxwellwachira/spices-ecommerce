...
class SppagebuilderModelMedia extends JModelList
{
	...
	public function insertMedia($title, $path, $media_attr='[]', $thumb='', $type='image') {
		...
		$values = array($db->quote($title), $db->quote($path), $db->quote($thumb), $db->quote($type), $db->quote($description), $db->quote($media_attr), $db->quote($title), $db->quote('com_sppagebuilder'), $db->quote( JFactory::getDate('now') ), JFactory::getUser()->id);
		$query
