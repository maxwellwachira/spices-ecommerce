...
class J2Product extends JObject{
	...
	*/
	...
	public function getCheckoutLink(&$product) {
		$product->checkout_link = JRoute::_('index.php?option=com_j2store&view=carts');
		J2Store::plugin()->event('ProductCheckoutLink', array($product));
	}

	public function getProductLink(&$product) {
		$product->product_link = JRoute::_('index.php?option=com_j2store&view=products&task=view&id='.$product->j2store_product_id);
