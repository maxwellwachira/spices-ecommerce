		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				$.each( json['error'], function( key, value ) {
					if (value) {
						$('#billing-address #'+key).after('<span class="j2error">' + value + '</span>');
					}
					//alert( key + ": " + value );
					});
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				$.each( json['error'], function( key, value ) {
					if (value) {
						$('#shipping-address #'+key).after('<span class="j2error">' + value + '</span>');
					}
				});
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				$.each( json['error'], function( key, value ) {
					if (value) {
						$('#shipping-payment-method #'+key).after('<span class="j2error">' + value + '</span>');
					}
				});
