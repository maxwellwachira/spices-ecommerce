					...
					<div class="j2store-cart-info-link" data-hover="dropdown" data-toggle="dropdown">
						...
						<?php if($list['product_count'] > 0): ?>
								...
								<span class="j2store-cart-count">
									<?php echo JText::sprintf($list['product_count'], $currency->format($list['total'])); ?>
								</span>
						...
						<?php if( $params->get('enable_checkout') ||  $params->get('enable_view_cart') ):?>
							<div class="j2store-cart-nav">
								<?php if($params->get('enable_checkout')):?>
									<a class="btn btn-dark btn-large"  href="<?php echo $checkout_url;?>">
...
jQuery('.j2store-minicart-button').mouseenter(function() {
	 jQuery('#j2store_cart_item_<?php echo $module->id; ?>').stop(false,false).fadeTo( "fast", 1.0 );
});

jQuery('.j2store-minicart-button').mouseleave(function() {

	jQuery('#j2store_cart_item_<?php echo $module->id; ?>').stop(true,true).fadeTo( "slow", 0.0, function() {
	jQuery( this )
		.css({
            "display": "none",
        });
